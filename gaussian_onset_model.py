...
def pretrain_dataset(subjects, onset_dict):
    ...
    for subj in subjects:
        ...
        for trial_idx in range(labels.shape[0]):
            ...
            labels = create_labels(n_samples, onsets)

            #trim if labels and data sequences somehow don't match
            if labels.shape[0] != raw_windows.shape[0]:
                nmin = min(labels.shape[0], raw_windows.shape[0])
    ...
    for subj in subjects:
        ...
        for trial_idx in range(labels.shape[0]):
            ...
            if labels.shape[0] != windows.shape[0]:
                min_size = min(labels.shape[0], windows.shape[0])
            ...
            temp_labels.append(labels.astype(np.float32))
