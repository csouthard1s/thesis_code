...
# def build_chroma_eegnet(chans, samples, nb_classes, pretrained_weights):
...
# def build_onset_eegnet(chans, samples, nb_classes, pretrained_weights):

#     base = EEGNet(nb_classes=nb_classes, Chans=chans, Samples=samples)
#     flatten_layer = base.get_layer('flatten').output #we don't necessarily want eegnet classifier output, so take layer before that which is flatten layer
#     out = tf.keras.layers.Dense(nb_classes, activation='softmax')(flatten_layer) #we used linear activation for onset, but this is just referring to the out layer of eegnet (which we cut off before), so softmax is fine
#     model = Model(inputs=base.input, outputs=out)
#     model.load_weights(pretrained_weights)
#     feat_model = Model(inputs=model.input, outputs=model.get_layer('flatten').output)
#     return feat_model
...
# eegnet_model_onset = build_onset_eegnet(chans=64, samples=128, nb_classes=1, pretrained_weights='/content/pretrain_onset.keras')
...
# eegnet_model_chroma = build_chroma_eegnet(chans=64, samples=128, nb_classes=12, pretrained_weights='/content/pretrain_chroma.keras')
# @register_keras_serializable()
...
onset_model = load_model("finetune_onset.keras")
chroma_model = load_model("finetune_chroma.keras")
